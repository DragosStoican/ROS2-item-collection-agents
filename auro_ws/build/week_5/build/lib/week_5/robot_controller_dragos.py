...
from auro_interfaces.msg import ItemList
        ...
        match self.state:
            ...
            case State.TURNING:
                ...
                if len(self.items) > 0:
                    ...
                    self.get_logger().info("Detected item!")
            ...
            case State.COLLECTING:
                if self.items == []:
                    ...
                    self.goal_distance = random.uniform(1.0, 2.0)
                    self.state = State.FORWARD
                    self.get_logger().info(f"Collecting item and returing to random movement, driving forward by {self.goal_distance:.2f} metres")
                else:
                    x = self.items[0].x
                    diameter = self.items[0].diameter
                    msg = Twist()
                    msg.linear.x = LINEAR_VELOCITY
                    if math.fabs(x) > 5 and diameter < 350:
                        msg.linear.x *= 0.5
                        if x > 0:
                            msg.angular.z = TURN_LEFT * ANGULAR_VELOCITY * 0.15
                            # self.get_logger().info("Turning Left to head towards item!")
                        else:
                            msg.angular.z = TURN_RIGHT * ANGULAR_VELOCITY * 0.15
                            # self.get_logger().info("Turning Right to head towards item!")
